...
if not os.path.exists('drive/MyDrive/Checkpoint_Task_4'):
  os.mkdir('drive/MyDrive/Checkpoint_Task_4')

def save_model(epoch, model, optimizer, scheduler, f1_list, c_f1):
  filename = 'drive/MyDrive/Checkpoint_Task_4/best_ch.pt'
  torch.save(
      {'epoch': epoch,
...
for epoch in range(start_epoch, params['epochs']):
  ...
  if early_stop(all_f1, c_f1, patience):
    break
  else:
